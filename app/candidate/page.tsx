...
export default function ApplicationsPage() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fetch recent jobs
        const recentJobsResponse = await jobService.getJobs({ limit: 3 });
        setRecentJobs(recentJobsResponse.jobs || []);
        
        // Fetch saved jobs
        const savedJobsResponse = await jobService.getSavedJobs();
        setSavedJobs(savedJobsResponse.jobs || []);
      } catch (error) {
  ...
  const employerJobs = recentJobs.filter(job => job.employerId === user?.id);
